%{
gustometer2 Function Documentation
==================================

**Purpose:**
The `gustometer2` function executes a behavioral training protocol using the Bpod system, where trials are 
randomly assigned one of four types. Each trial involves delivering a stimulus, collecting responses, and 
administering rewards or punishments based on the trial type.

**Global Variables:**
- `BpodSystem`: A global structure used by the Bpod system to manage trial data, protocol settings, 
  and system status.
...
2. **Define Trials:**
   - **Randomize Trial Types:**
     - Creates an array `TrialTypes` with randomly assigned values from 1 to 4.
   ...
   - **Loop Over Trials:**
     ...
     - In each iteration:
       ...
       - **Define State Machine:**
         ...
         - Defines states:
           - `WaitForPoke`: Waits for a poke in Port 2 to transition to `DeliverStimulus`.
...
function gustometer2
    ...
    %% Define trials
    TrialTypes = round(rand(1,MaxTrials) * 3) + 1; % Randomly interleaved trial types 1, 2, 3, and 4
    ...
    for currentTrial = 1:MaxTrials
        ...
        switch TrialTypes(currentTrial)
            ...
            case 3
                LeftAction = 'Punish'; RightAction = 'Reward';
                StimulusOutputActions = {'ValveModule1', 5}; % Action to open valve for trial type 3
                RewardOutputActions = {'ValveModule1', 1}; % Action to deliver reward
                PunishOutputActions = {'LED', 1};
            case 4
                LeftAction = 'Punish'; RightAction = 'Reward';
                StimulusOutputActions = {'ValveModule1', 6}; % Action to open valve for trial type 4
                RewardOutputActions = {'ValveModule1', 1}; % Action to deliver reward
                PunishOutputActions = {'LED', 1};
        end
